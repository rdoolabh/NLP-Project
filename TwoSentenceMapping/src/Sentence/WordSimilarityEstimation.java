...
public class WordSimilarityEstimation {
  ...
  // estimate the common ancestor height  
  private void estimateHeight(Dictionary dictionary, WordPair wp) throws JWNLException,
  ...
  // estimate the shortest path of two word
  private void estimateLength(WordPair wp) {
    length = shollowestList.getDepth();
  }

  // convert to the porter stemmer string
  ...
  // compute the word similarity score
  public void computeSimilarity(Dictionary dictionary, WordPair wp) {
    
    //void function. Set the result to wp.simanticScore
    ...
    double f_h = (heightTerm1 - heightTerm2) / (heightTerm1 + heightTerm2);
    wp.setScore(f_l * f_h);
